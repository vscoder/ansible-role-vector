...
function config_url {
    echo "https://repositories.timber.io/public/vector/cfg/setup/config.deb.txt?distro=${distro}&codename=${codename}&version=${version}&arch=${arch}" | sed 's/ /%20/g'
}

function check_fetch_config {
    local text="Checking if upstream install config is OK ..."
    echo_running "$text"
    echo_running "$(config_url)"
    exit 1
...
function import_gpg_key {
    local config_url="$(config_url)"
    local text="Config url '${config_url}' 'timber/vector' repository GPG key into apt ..."
    echo_running "$text"
    
    curl -1sLf "https://repositories.timber.io/public/vector/cfg/gpg/gpg.3543DB2D0A2BC4B8.key" | apt-key add - &>$tmp_log
